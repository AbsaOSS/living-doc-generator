...
def test_get_exporter_liv_doc_regime_mdoc():
    exporter = ExporterFactory.get_exporter(Regime.LIV_DOC_REGIME, Format.MDOC.value)
    assert isinstance(exporter, MdocExporter)

def test_get_exporter_invalid_regime():
    exporter = ExporterFactory.get_exporter("INVALID_REGIME", Format.MDOC.value)
    assert exporter is None

def test_get_exporter_invalid_format():
    exporter = ExporterFactory.get_exporter(Regime.LIV_DOC_REGIME, "INVALID_FORMAT")
    assert exporter is None

def test_get_exporter_invalid_regime_and_format():
    exporter = ExporterFactory.get_exporter("INVALID_REGIME", "INVALID_FORMAT")
    assert exporter is None
